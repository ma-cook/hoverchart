...
async function runUnloadingTest() {
  ...
  log(
    `Initial state: ${initialObjectCount} objects, ${initialState.loadedCells.length} cells loaded`
  );
  ...
  if (initialObjectCount === 0) {
    log(
      'No objects found! Please add some objects first and refresh the page.',
      'warning'
    );
    return;
  }
  ...
  log(
    `Final state: ${finalObjectCount} objects, ${finalState.loadedCells.length} cells loaded`
  );

  // Step 4: Analyze results
  const objectsRemoved = initialObjectCount - finalObjectCount;
  const cellsChanged =
    initialState.loadedCells.length !== finalState.loadedCells.length;

  log(
    `Objects removed: ${objectsRemoved} (${initialObjectCount} → ${finalObjectCount})`
  );
  log(
    `Cells changed: ${cellsChanged} (${initialState.loadedCells.length} → ${finalState.loadedCells.length})`
  );
  ...
  if (objectsRemoved > 0 && cellsChanged) {
    ...
    log(
      '🎉 SUCCESS: Objects were properly unloaded when cells changed!',
      'success'
    );
    log(
      'The fix is working correctly - objects added before page refresh are being tracked and unloaded properly.',
      'success'
    );
  } else if (objectsRemoved === 0 && !cellsChanged) {
    log(
      'ℹ️ No cells were unloaded, so no objects should be removed. This is expected behavior.',
      'info'
    );
    log(
      'Try moving the camera even further away or adding objects in more distant cells.',
      'info'
    );
  } else if (objectsRemoved === 0 && cellsChanged) {
    log(
      '❌ ISSUE: Cells changed but no objects were removed. The issue may still exist.',
      'error'
    );
    log(
      'Objects that were added before page refresh may not be properly tracked for unloading.',
      'error'
    );
  } else {
...
if (getObjectCount() > 0) {
  ...
  log(
    'No objects detected. Please add some objects and run the test manually:',
    'info'
  );
  log(
    'window.testUnloadingFix.runTest = runUnloadingTest; window.testUnloadingFix.runTest();',
    'info'
  );
