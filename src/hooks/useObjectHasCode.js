 ...
 * hydration) OR the ContentStore's repo: entries change, so the `</>` button
...
export function useObjectHasCode(objectData) {
  ...
  return useMemo(() => {
    ...
    void contentVersion;
    return objectHasCode(objectData, { repoFileContents });
  }, [objectData, repoFileContents, contentVersion]);
}