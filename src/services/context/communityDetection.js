 ...
 */
export function detectCommunities(graphs, options = {}) {
  ...
  if (allNodeIds.size < opts.minCommunitySize) {
    const single = new Map();
    let i = 0;
    for (const id of allNodeIds) single.set(id, i++);
    return single;
  }
  ...
  for (let iter = 0; iter < opts.maxIterations; iter++) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      communityInternalWeight[currentComm] -= adjacency[i].size > 0
        ? Array.from(adjacency[i].entries()).filter(([j]) => community[j] === currentComm).reduce((s, [, w]) => s + w, 0)
        : 0;
