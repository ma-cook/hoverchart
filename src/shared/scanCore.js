...
import { computeSymbolRanges, languageFromFilePath } from '../services/symbolRanges';
import { detectCommunitiesSync } from '../services/context/communityDetection';
 ...
 */
export const TREE_SITTER_EXTENSIONS = [
  { test: /\.go$/i,                key: 'go' },
  { test: /\.rs$/i,                key: 'rust' },
...
export const getTreeSitterLanguage = (filePath) => {
 ...
 * @param {Array} structure - Repository file list (injected via runRepositoryScan options)
 * @param {Function} fetchFile - (relativePath) => Promise<string|null> raw file body
 * @returns {Promise<'react'|'nextjs'|'vue'|'python'|'vanilla'>}
 */
export const detectRepoType = async (structure, fetchFile) => {
 ...
 * @returns {string}
 */
export const sanitizeNodeId = (name) => {
 ...
 */
export const runRepositoryScan = async (owner, repoName, options = {}) => {
  ...
  try {
    ...
    const structure = options.structure || options.preFilteredFiles;
    if (!structure) {
      throw new Error('runRepositoryScan: no repo structure provided (options.structure or options.preFilteredFiles required)');
    }
    console.log(`📁 Using ${options.preFilteredFiles ? 'pre-filtered' : 'provided'} file list: ${structure.length} files`);
    ...
    const repoType = options.repoType || await detectRepoType(structure, options.fetchFile);
    ...
    if (isVanillaRepo) {
      try {
const pkgContent = await options.fetchFile('package.json');
        if (pkgContent) {
          const pkg = JSON.parse(pkgContent);
    ...
    if (repoType === 'react' || repoType === 'nextjs' || repoType === 'vanilla') {
      try {
        const tsconfigContent = await options.fetchFile('tsconfig.json');
        const jsconfigContent = !tsconfigContent ? await options.fetchFile('jsconfig.json') : null;
    ...
    const fetchWorker = async () => {
      while (fetchIdx < filesToProcess.length) {
        ...
        const fileContent = await options.fetchFile(file.path);
      ...
      try {
        ...
        if (tsSourceFiles.size > 0) {
          ...
          if (moduleResolver) {
            ...
            try {
              tsconfigForTS = await options.fetchFile('tsconfig.json');
            } catch { /* ignore */ }
          }

          importPerf.mark('scan: L2 TypeScript analysis starting');
          const tsAnalysis = await options.runTypeScriptAnalysis(tsSourceFiles, tsconfigForTS);
 ...
 */
export const generateMerfolkMarkdown = ({
  repoName,
  repoFileContents = {},
